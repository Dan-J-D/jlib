...
unsigned int jlib_unescape_str_len(char *str, unsigned int str_len)
{
	unsigned int len = 0;
	for(unsigned int i = 0; str[i] != '\0' && i < str_len; i++)
	{
		if(str[i] == '\\')
		{
			if(i + 1 > str_len)
...
void jlib_unescape_str(char *str, char *buf, unsigned int buf_len)
{
	for(unsigned int i = 0; str[i] != '\0' && i < buf_len; i++)
	{
		if(str[i] == '\\')
		{
			if(i + 1 > buf_len)
...
char *jlib_decode(jlib_val *val, char *buf, unsigned int buf_len)
{
	...
	case '"':
	{
		...
		val->type = JLIB_TYPE_STRING;
		unsigned int len = jlib_unescape_str_len(buf + 1, buf2 - buf - 1);
		if(!len)
			return NULL;
		...
		memset(val->str, 0, len + 1);
		val->need_free = 1;
		jlib_unescape_str(buf + 1, val->str, len);
